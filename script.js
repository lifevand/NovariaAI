...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // START: Model Selector Variables and Elements
    const modelSelectorContainer = document.getElementById('modelSelectorContainer');
    const currentModelToggle = document.getElementById('currentModelToggle');
    const currentModelLogo = document.getElementById('currentModelLogo'); // Logo bisa sama atau beda per model
    const currentModelName = document.getElementById('currentModelName');
    const modelOptionsList = document.getElementById('modelOptionsList');
    const modelToggleArrow = document.getElementById('modelToggleArrow');

    // Daftar model yang tersedia, sesuaikan ID dengan NAMA MODEL API YANG VALID
    // 'name' adalah yang tampil di UI, 'id' adalah yang dikirim ke backend
    // 'logo' adalah path ke gambar logo
    const availableModels = [
        { id: 'gemini-1.5-flash',       name: 'gemini-1.5-flash',       logo: './gemini-color.webp' }, // Default
        { id: 'gemini-2.5-flash',       name: 'gemini-2.5-flash',       logo: './gemini-color.webp' },
        { id: 'gemini-2.0-flash',       name: 'gemini-2.0-flash',       logo: './gemini-color.webp' },
        { id: 'gemini-2.0-flash-lite',  name: 'gemini-2.0-lite',        logo: './gemini-color.webp' }, // Tampilan disingkat
        { id: 'gemini-2.0-flash-preview-image-generation', name: 'gemini-gen-image', logo: './gemini-color.webp' } // Tampilan disingkat
    ];
    // Pastikan default model ada di daftar availableModels
    let selectedModelId = localStorage.getItem('novaai_selectedModel') || 'gemini-1.5-flash';
    if (!availableModels.find(m => m.id === selectedModelId)) {
        selectedModelId = 'gemini-1.5-flash'; // Fallback jika model di localStorage tidak valid
    }

    let isModelSelectorOpen = false;
    // END: Model Selector Variables and Elements

// ... (sisa script.js tetap sama, termasuk fungsi populateModelOptions, updateCurrentModelDisplay, dll.) ...
// Fungsi populateModelOptions akan otomatis menggunakan 'name' untuk tampilan dan 'id' untuk data.
// Fungsi updateCurrentModelDisplay juga akan menggunakan 'name' untuk tampilan.
    ...
    async function generateRealAIResponse(userMessage, files = []) {
        ...
        try {
            // Use the globally selected model ID
            const payload = { userMessage, model: selectedModelId }; // MODIFIED HERE
            ...
            setTimeout(() => {
                ...
                responseText.replace(codeBlockRegex, (match, language, code, offset) => {
                    ...
                    const sanitizedText = plainText.replace(/</g, "<").replace(/>/g, ">"); // Sanitized
                    finalHtmlContent += `<span>${sanitizedText}</span>`;

                    const lang = language || 'text';
                    const sanitizedCode = code.trim().replace(/</g, "<").replace(/>/g, ">"); // Sanitized
                    const codeHtml = `
                ...
                if (remainingText) {
                    const sanitizedRemainingText = remainingText.replace(/</g, "<").replace(/>/g, ">"); // Sanitized
    ...
    // Call setupRippleEffects after dynamic elements are added or initially
    ...
    setupRippleEffects(); // Initial call for static elements
